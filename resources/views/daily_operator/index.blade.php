        ...
        <main>
            ...
            <div class="container-xl px-4 mt-n10">
                <div class="card mb-3">
                    ...
                    <div class="card-body">
                        <form action="{{ route('operator.filter') }}" method="POST">
                            @csrf
                            <div class="row mb-3">
                                <div class="col-md-6">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        @endif
                        {{--  <a class="btn btn-outline-green float-right" href="{{ route('operator.create') }}" role="button"><i data-feather="plus-circle"></i> &nbsp ADD </a>  --}}
                        <!-- Print PDF -->
                            ...
                            <table class="table table-bordered table-striped" id="simpleDatatables">
                                ...
                                <tbody>
                                    @foreach ($operator as $op)
                                        <tr>
                                            <td>{{ $op->name }}</td>
                                            <td>{{ now()->parse($op->jamMasuk)->translatedFormat('l, d F Y H:i A') }}
                                                -
                                                {{ now()->parse($op->jamKeluar)->translatedFormat('l, d F Y H:i A') }}
                                            </td>
                                            <td>{{ $op->no_lambung }}</td>
                                            <td>{{ $op->waktu }}</td>
                                            {{-- <td>{{ $op->nama_lokasi }}</td> --}}
                                            <td>{{ $op->nama_lokasi }}</td>
                                            <td>{{ $op->hm }}</td>
                                            ...
                                            @if (empty($op->photo_hm) || $op->photo_hm === 'null')
                                                <td class="text-center"><img
                                                        src="{{ asset('storage/camera_default.png') }}" height="60"
                                                        width="60" alt=""></td>
                                            @else
                                                <td><img src="{{ asset('storage/photoHm/' . $op->photo_hm) }}"
                                            ...
                                            <td class="text-center">
                                                <a class="btn btn-warning btn-sm" title="Edit"
                                                    href="{{ route('operator.edit', $op->id) }}">
                                                    <i data-feather="edit"></i>&nbsp;
                                                    Edit
                                                </a>
                                                {{--  <form method="post" action="{{ route('operator.destroy', $op->id) }}">
                                                    @csrf
                                                    @method('DELETE')
