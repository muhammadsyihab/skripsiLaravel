    ...
    <div id="layoutSidenav_content">
        ...
        <main>
            ...
            <div class="container-xl px-4 mt-n10">
                <div class="card">
                    <div class="card-header">Edit Daftar Perbaikan</div>
                    <div class="card-body">
                        <form method="post" action="{{ route('service.update', $serviceById->id) }}"
                            ...
                            {{-- <div class="row mb-4">
                                <div class="col">
                                    <div class="mb-3">
                                        <label for="status_unit">Sparepart</label>
                                        <select class="form-control form-control-solid pencarian" id="sparepart_id"
                                            name="sparepart_id" required>
                                            <option value="" selected disabled>Sparepart</option>
                                            @foreach ($spareparts as $sparepart)
                                                <option value="{{ $sparepart->id }}"{{ old('sparepart_id', $serviceById->master_sparepart_id) == $sparepart->id ? ' selected' : '' }}>{{ $sparepart->nama_item }}</option>
                                            @endforeach
                                        </select>
                                        @error('sparepart_id')
                            ...
                            <div class="row">
                                <div class="col">
                                    <div class="mb-3">
                                        ...
                                        min="0" value="{{ old('hm', $serviceById->hm) }}" required>
                                        @error('hm')
                                            <div class="text-danger mt-5">Tolong isi dengan benar</div>
                                        @enderror
                                    </div>
                            ...
                            <div class="row">
                                <div class="col">
                                    <div class="mb-3">
                                        <label for="spareparts">Sparepart{{ $serviceById->nama_sparepart }}</label><br>
                                    <select class="form-control form-control-solid pencarian" id="spareparts[]"
                                        name="spareparts[]" multiple="multiple" required>
                                            ...
                                            @foreach ($spareparts as $sparepart)
                                                <option value="{{ $sparepart->id }}"{{ (in_array($sparepart->nama_item, $selected)) ? ' selected' : '' }}>{{ $sparepart->nama_item }} - {{ $sparepart->part_number }}</option>
                                            @endforeach
                                        </select>
                                        @error('sparepart_id')
                                            <div class="text-danger mt-5">Tolong isi dengan benar</div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control units pencarian" id="master_unit_id" name="master_unit_id" required>
                                            ...
                                            @foreach ($units as $unit)
                                                <option value="{{ $unit->id }}"{{ old('master_unit_id', $serviceById->master_unit_id) == $unit->id ? ' selected' : '' }}>
