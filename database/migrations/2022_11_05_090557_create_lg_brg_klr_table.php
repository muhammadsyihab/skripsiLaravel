<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateLgBrgKlrTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('lg_brg_klr', function (Blueprint $table) {
            $table->id();
            $table->foreignId('master_sparepart_id')->references('id')->on('master_sparepart')->onDelete('cascade')->nullable();
            $table->foreignId('users_id')->references('id')->on('users')->onDelete('cascade')->nullable();
            $table->integer('qty_keluar');
            $table->integer('status')->default(0);
            $table->integer('penerima');
            $table->float('hm_odo', 15, 2)->default(0);
            $table->date('tanggal_keluar');
            $table->integer('estimasi_pengiriman');
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('lg_brg_klr');
    }
}
